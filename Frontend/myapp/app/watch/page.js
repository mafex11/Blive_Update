...
const WatchStreamPage = () => {
  ...
  useEffect(() => {
    const fetchActiveStreams = async () => {
      try {
        const response = await fetch('/api/streams/active', { cache: 'no-store' }); // Disable cache on the client side
    ...
    fetchActiveStreams();
  }, []); // Fetch data every time the page loads
        ...
        <div className="flex flex-wrap gap-4">
          {activeStreams.map((stream, index) => (
            <Link href={`/stream/${stream.playbackId}`} key={index} passHref>
              <div className="p-4 bg-blue-500 text-white rounded-lg shadow-md hover:shadow-lg hover:bg-blue-600 transition transform hover:-translate-y-1 cursor-pointer">
